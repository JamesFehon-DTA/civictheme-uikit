...
A slider rotates a small number of featured items in a single visual frame.
...
- **for the chapter list of a report or plan** – chapters are sequential reading; a slider hides the structure from search and from screen readers paginating the page. Use a card grid of **navigation cards** instead.
- **for headline commitments of a strategy or plan** – readers need to compare commitments at a glance, not click through them. Use a card grid or, for a single dominant commitment, a **callout**.
...
- **as the primary page hero on standard content pages** – a single static **banner** carries more weight and demands no interaction.
...
Where every slide is genuinely important, the slider is the wrong component – use a card grid so all items are visible. Where only the first slide is important, use a **banner** or a **promo** and drop the rotation.
...
- **Banner** – a single static page header. The right choice when one item carries the page.
- **Promo** – a single featured item with image and call-to-action. The right choice when a slider would otherwise have one slide.
- **Campaign** – a major editorial feature. The right choice for the kind of marketing surface a slider often gets misused for.

## Related patterns

- **Card selection** – which card type to use when a slider is the wrong answer.
