{/* Editorial guidance ported from the digital.gov.au design library (dga-dl /src/content). */}
{/* Internal /patterns and /templates links demoted to plain text – they do not resolve in Storybook. */}

import { Meta, Canvas, Controls } from '@storybook/addon-docs/blocks';
import * as Stories from './side-navigation.stories';

<Meta of={Stories} name="Docs" />

# Sub-nav

Secondary navigation for moving between pages within a section.

<Canvas of={Stories.SideNavigation} />

<Controls of={Stories.SideNavigation} />

Use a sub-nav to help users move between the pages within a section without returning to the top-level navigation. It appears as a persistent list of links alongside the page content.

## When to use

- the current section contains multiple pages that users are likely to navigate between
- the section has a clearly bounded scope – for example, a program, a service area, or a policy topic
- users need to understand what else is available in the section while reading a page

## When not to use

- for in-page navigation (links to sections within the same page) – use **Table of contents** instead
- as the primary site navigation
- on standalone pages that do not belong to a defined section
- when the section contains only one or two pages – a sub-nav adds navigation overhead without benefit

## Related components

- **Table of contents** – use for in-page navigation generated from H2 headings on the current page.
- **Filter sidebar** – use to filter search results or catalogue listings, not for page navigation.

## Related patterns

- **Section navigation** – guidance on when to use a sub-nav strip versus side navigation based on section depth and size.
